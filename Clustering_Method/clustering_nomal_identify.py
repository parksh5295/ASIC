...
def clustering_nomal_identify(data_features_for_clustering, original_labels_aligned, clusters_assigned, num_total_clusters, global_known_normal_samples_pca=None):
    ...
    if global_known_normal_samples_pca is not None:
        print(f"[DEBUG CNI] Received 'global_known_normal_samples_pca' - Shape: {global_known_normal_samples_pca.shape}")
    else:
        ...
        if original_labels_aligned is not None and data_features_for_clustering.shape[0] == original_labels_aligned.shape[0]:
            ...
            if num_temp_known_normal > 1:
                sample_size = int(num_temp_known_normal * 0.8)
    ...
    for cluster_id in range(num_total_clusters):
        ...
        if current_cluster_features.size == 0 or known_normal_samples_features_to_compare.size == 0:
            num_normal_in_cluster = 0
        else:
            try:
                # Compare with the provided global known normal samples
